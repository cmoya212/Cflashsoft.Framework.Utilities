...
namespace Cflashsoft.Framework.Optimization
{
    ...
    public static class MemoryCacheExtensions
    {
        ...
        /// Return an item from the memory cache or insert it using the provided function.
        /// </summary>
        public static Task<T> SyncedGetOrSetAsync<T>(this MemoryCache cache, string key, Func<Task<T>> getValue, int expirationSeconds = 0, string regionName = null)
            where T : class
        {
            return SyncedGetOrSetAsync<T>(cache, key, getValue, expirationSeconds > 0 ? DateTime.Now.AddSeconds(expirationSeconds) : DateTimeOffset.MinValue, regionName);
        }

        /// <summary>
        /// Return an item from the memory cache or insert it using the provided function.
        /// </summary>
        public static async Task<T> SyncedGetOrSetAsync<T>(this MemoryCache cache, string key, Func<Task<T>> getValue, DateTimeOffset absoluteExpiration, string regionName = null)
            where T : class
        {
            T result = (T)cache.Get(key);
